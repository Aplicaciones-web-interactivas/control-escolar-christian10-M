...
<h1 class="text-2xl font-bold mb-6">Inscripciones</h1>

<h2 class="text-xl font-semibold mb-4">Oferta de Cursos</h2>
...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
    ...
    @foreach($grupos as $grupo)
        ...
        <div class="p-5 rounded-xl shadow transition
            ...
            <h3 class="text-lg font-bold mb-2">{{ $grupo->nombre }}</h3>
            <p class="text-gray-600 text-sm">{{ $grupo->horario->materia->nombre }}</p>
            <p class="text-gray-600 text-sm mb-2">{{ $grupo->horario->maestro->nombre }}</p>
            ...
            @else
                <form action="/inscribirse/{{ $grupo->id }}" method="POST"
                    onsubmit="return confirm('¿Seguro que deseas inscribirte?')">
                    @csrf
...
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    @forelse($misInscripciones as $inscripcion)
        ...
        <div class="bg-blue-50 border border-blue-200 p-5 rounded-xl shadow">
            ...
            <h3 class="text-lg font-bold mb-2">{{ $inscripcion->grupo->nombre }}</h3>
            <p class="text-gray-700 text-sm">{{ $inscripcion->grupo->horario->materia->nombre }}</p>
            <p class="text-gray-700 text-sm mb-2">{{ $inscripcion->grupo->horario->maestro->nombre }}</p>
            <p class="text-xs text-gray-500 mb-3">
            ...
            <form action="/inscripciones/{{ $inscripcion->id }}" method="POST"
                onsubmit="return confirm('¿Deseas darte de baja?')">
                @csrf
                @method('DELETE')
                <button class="w-full bg-red-600 hover:bg-red-700 text-white py-2 rounded transition">
                    Dar de baja
                </button>
            </form>
    ...
    @empty
        <p class="text-gray-500">No tienes inscripciones aún.</p>
    @endforelse
